...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    /* 对商品信息赋值 */
    $cart_goods = cart_goods($flow_type,$_GET['rec_id']); // 取得商品列表，计算合计
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	if(!$_POST['is_check']){
		echo "<script>alert('请勾选以了解保险条款！');history.go(-1);</script>";exit;
	}
	$rec_id=$_POST['rec_id'];
    /* 检查购物车中是否有商品 */
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
...
elseif ($_REQUEST['step'] == 'cx_insure_cl')
{
	...
	$smarty->assign('djq',            $djq);  //单交强产品
	$smarty->assign('dzb',            $dzb);  //大众版产品
	$smarty->assign('jqb',            $jqb);  //加强版产品
	
	$smarty->assign('djq_xz',         $djq_xz);  //单交强险种
	$smarty->assign('dzb_xz',         $dzb_xz);  //大众版险种
	$smarty->assign('jqb_xz',         $jqb_xz);  //加强版险种
	
	$smarty->assign('djq_total',         $djq_total);  //单交强险种合计保费
	$smarty->assign('dzb_total',         $dzb_total);  //大众版险种合计保费
	$smarty->assign('jqb_total',         $jqb_total);  //加强版险种合计保费
}
...
elseif ($_REQUEST['step'] == 'js_price')
{
	...
	if($xz == 'clss'){
		...
		$_SESSION['clss_bj']=$bjmp_price;      //不计免赔费用
    ...
    if($xz == 'dszr'){
		...
		$_SESSION['dszr_bj']=$bjmp_price;   //不计免赔费用
	...
	if($xz == 'sjzw'){
		...
		$_SESSION['sjzw_bj']=$bjmp_price;    //不计免赔费用
	...
	if($xz == 'ckzw'){
		...
		$_SESSION['ckzw_bj']=$bjmp_price;   //不计免赔费用
	...
	if($xz == 'qdx'){
		...
		$_SESSION['qdx_bj']=$bjmp_price;   //不计免赔费用
...
elseif ($_REQUEST['step'] == 'tj_insure')
{
	...
	if(@$_POST['cat_id']){
		...
		if($_POST['clss']){
			...
			$tbxz[]=array('xz_name'=>'车辆损失险','tb_price'=>$_POST['clss'],'is_mp'=>$clss_bj,'price'=>($_SESSION['clss_bj']+$_SESSION['clss_price']));
		}
		if($_POST['dszr']){
			$dszr_bj=$_SESSION['dszr_bj'] ? 1 : 0;
			$tbxz[]=array('xz_name'=>'第三责任险','tb_price'=>$_POST['dszr'],'is_mp'=>$dszr_bj,'price'=>($_SESSION['dszr_bj']+$_SESSION['dszr_price']));
		}
		if($_POST['sjzw']){
			$sjzw_bj=$_SESSION['sjzw_bj'] ? 1 : 0;
			$tbxz[]=array('xz_name'=>'司机座位险','tb_price'=>$_POST['sjzw'],'is_mp'=>$sjzw_bj,'price'=>($_SESSION['sjzw_bj']+$_SESSION['sjzw_price']));
		}
		if($_POST['ckzw']){
			$ckzw_bj=$_SESSION['ckzw_bj'] ? 1 : 0;
			$tbxz[]=array('xz_name'=>'乘客座位险','tb_price'=>$_POST['ckzw'],'is_mp'=>$ckzw_bj,'price'=>($_SESSION['ckzw_bj']+$_SESSION['ckzw_price']));
		}
		if($_POST['qdx']){
			$qdx_bj=$_SESSION['qdx_bj'] ? 1 : 0;
			$tbxz[]=array('xz_name'=>'盗抢险','tb_price'=>$_POST['qdx'],'is_mp'=>$qdx_bj,'price'=>($_SESSION['qdx_bj']+$_SESSION['qdx_price']));
		}
		if($_POST['blps']){
			$blps=$_POST['blps'] ==1 ? '国产车' : '进口车';
			$tbxz[]=array('xz_name'=>'玻璃破碎险','tb_price'=>$blps,'is_mp'=>0,'price'=>$_SESSION['blps_price']);
		}
		if($_POST['zdzx']){
			$zdzx=$_POST['zdzx'] ==1 ? '国产车' : '进口车';
			$tbxz[]=array('xz_name'=>'指定专修险','tb_price'=>$zdzx,'is_mp'=>0,'price'=>$_SESSION['zdzx_price']);
		}
		$tbxz_total=$_SESSION['xz_total'];
		$_SESSION['tbxz_info']=serialize($tbxz);
		$smarty->assign('custom',            1); //自定义
	}else{
...
elseif ($_REQUEST['step'] == 'cx_done')
{
	...
	if(!$_POST['is_check']){
		echo "<script>alert('请勾选车险投保服务协议！');history.go(-1);</script>";
		exit;
	}
	include_once('includes/lib_clips.php');
    include_once('includes/lib_payment.php');
