...
foreach($ad1 as $k=>$v){
    $ad1[$k]['ad_link']='/mobile'.$v['ad_link'];
}
...
/*合作伙伴*/
/*$sql1 = 'SELECT * FROM ' . $ecs->table("friend_link");
$link = $db->getAll($sql1);//print_r($link);exit;

foreach($link as $k=>$v){
    if(preg_match('/^d/',$v['link_logo'])){
        $link[$k]['link_logo'] = '/'.$v['link_logo'];
    }
}

$smarty->assign('link_list',      $link);*/
$sql = 'SELECT link_logo, link_name  FROM ' . $GLOBALS['ecs']->table('hzhb') . ' ORDER BY link_order';
...
//print_r($hz);
$smarty->assign('link_list',     $hz);
/*新闻中心*/
$sql3='SELECT cat_id FROM ' . $ecs->table("article_cat").'where parent_id=5';
$cat_id = $db->getAll($sql3);
for($i=0;$i<count($cat_id);$i++){
    $catid[$i]=$cat_id[$i][cat_id];
}
$catid=implode(',',$catid);
$sql2 = 'SELECT * FROM ' . $ecs->table("article").'where cat_id in ('.$catid.') ORDER BY article_id desc limit 0,4';
$new = $db->getAll($sql2);//print_r($link);exit;
foreach($new as $k=>$v){
    if($v['file_url']==null){
        $new[$k]['file_url']='images/upimg3.jpg';
    }
    $new[$k]['add_time']=local_date('Y/m/d', $v['add_time']);
}
