...
if($id==null){
    ...
    $cat_id='AND cat_id='.$id;
}
$sort=isset($_GET['sort'])?$_GET['sort']:'goods_id';
$order=isset($_GET['order'])?$_GET['order']:'ASC';
$like=isset($_POST['like'])?$_POST['like']:'';
$size=4;
$start=@$_GET['p'] ? ($_GET['p']-1)*$size : 0;
$sql='SELECT goods_id,cat_id,goods_name,goods_sn,salesnum,shop_price,goods_thumb FROM ' . $GLOBALS['ecs']->table('goods') . 'WHERE  goods_name LIKE "%'.$like.'%" '.$cat_id.' ORDER BY '.$sort.' '.$order.' limit '.$start.','.$size;
...
if(@$_GET['c']){
    ...
    if(!empty($goods)) {
        foreach ($goods as $k => $v) {
            $html .= "<li><p class=\"col-xs-3 photo\"><a href=\"#3\"><span><img src='/{$v['goods_thumb']}'/></span></a></p>";
            $html .= "<div class=\"col-xs-8 text\"><h3><a href=\"#3\">{$v['goods_name']}</a></h3>";
            $html .= "<p>编号：{$v['goods_sn']}  | 销量：{$v['salesnum']}份 </p><p>￥<span class=\"cor-red\">{$v['shop_price']}</span> </p>";
            $html .= "</div><div class=\"col-xs-1 art\"><img src=\"/images/izl-icon_48.png\" ></div>";
            $html .= "<input type=\"hidden\" class=\"last_id\" value=\"{$v['goods_id']}\"></li>";
        }
    }else{
        $html=false;
    }
