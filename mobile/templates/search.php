...
else
{
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price,g.goods_sn,g.salesnum, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
                "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .
            "ORDER BY $sort $order";
    $res = $db->SelectLimit($sql, 100, ($page - 1) * $size);
    ...
    while ($row = $db->FetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['salenum']      = $row['salesnum'];
