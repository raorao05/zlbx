...
if($cat['parent_id']==1){
	...
	$province = $db->getall("SELECT region_id,region_name FROM " . $ecs->table('region') . " WHERE region_type=1");
    $city = $db->getall("SELECT region_id,region_name FROM " . $ecs->table('region') . " WHERE parent_id=" . $pro_id);
    ...
    $smarty->display('category_cx1.dwt');
}else{
