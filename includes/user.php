    ...
    if(IS_POST){
       // echo $_REQUEST['name'].$_REQUEST['password'];
        if ($user->login($username, $password,isset($_POST['remember'])))
        {
            ...
            recalculate_price();
            if($_COOKIE['sopenid']){
                $sql = "DELETE * FROM " .$GLOBALS['ecs']->table('users').
                    " WHERE wx_open_id = '" . $_COOKIE['sopenid'] . "'";
                $GLOBALS['db']->query($sql);
                $sql = "UPDATE " .$GLOBALS['ecs']->table('users'). " SET".
                    " wx_open_id = '".$_COOKIE['sopenid']."'".
                    " WHERE user_id = '" . $_SESSION['user_id'] . "'";
                $GLOBALS['db']->query($sql);
            }

            show_message('绑定成功' , array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act,'user.php'), 'info');
        }
        else
        {
            @$_SESSION['login_fail'] ++ ;
...
elseif ($action == 'act_register')
{
    ...
    else
    {
		...
		if($num>0){
			echo "<script language='javascript'>alert('此登录名已注册！');history.go(-1);</script>";exit;
		}
		...
		if (isset($_FILES['sfz_pic']['error'])) // php 4.2 版本才支持 error
		{
			// 最大上传文件大小
			...
			{
				show_message(sprintf('图片文件太大了(最大值：1M)，无法上传。', $php_maxsize), $_LANG['profile_lnk'], 'user.php?act=register', 'info');
			}
			elseif ($_FILES['sfz_pic']['error'] == 2)
			{
				show_message(sprintf('图片文件太大了(最大值：1M)，无法上传。', $htm_maxsize), $_LANG['profile_lnk'], 'user.php?act=register', 'info');
			}
		...
		if (!empty($_FILES['sfz_pic']['name']))
		{
			...
			$original_img = $image->upload_image($_FILES['sfz_pic'], 'sfz_pic', $img_name); // 原始图片
...
elseif ($action == 'act_edit_profile')
{
   ...
   /* else
    ...
    $profile  = array(
		...
		'alias'    => isset($_POST['alias']) ? trim($_POST['alias']) : '',  //姓名
		'zj_number'    => isset($_POST['zj_number']) ? trim($_POST['zj_number']) : '',  //姓名
		'parent_id'    => $parent_id ? $parent_id : '',  //姓名
    ...
    if(@$_GET['c']){
        ...
        if(!empty($orders)) {
            foreach ($orders as $k => $v) {
                $html .= "<li><div class=\"time\"><span class=\"xdtime\">下单时间：{$v['order_time']}</span>金额：<span class=\"red\">{$v['total_fee']}</span></div>";
                ...
                $html .= "<div class=\"text mar-ai\"><a href=\"user.php?act=order_detail&order_id={$v['order_id']}\"><p>订单号：{$v['order_sn']}</p><p>订单状态：<span class=\"corhs\">{$v['order_status']}</span></p>";
                $html .= "</a></div><div class=\"art\"></div></div><div class=\"czsc\"><div class=\"but off\">{$v['handler']}</div>";
                $html .= "<div class=\"but on\"><a href=\"user.php?act=order_detail&order_id={$v['order_id']}\" title=\"订单详情\">订单详情</a></div></div></li>";
            }
        }else{
            $html=false;
        }
